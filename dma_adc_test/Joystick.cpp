...
static uint32_t chnl2 = 0;  // DMA channel
...
void adc_dma(void *rxdata,  size_t hwords,int _chnl) 
{
    uint32_t temp_CHCTRLB_reg;

    DMAC->CHID.reg = DMAC_CHID_ID(_chnl);
    DMAC->CHCTRLA.reg &= ~DMAC_CHCTRLA_ENABLE;
    DMAC->CHCTRLA.reg = DMAC_CHCTRLA_SWRST;
    DMAC->SWTRIGCTRL.reg &= (uint32_t)(~(1 << _chnl));
    ...
    memcpy(&descriptor_section[_chnl],&descriptor, sizeof(dmacdescriptor));
...
void adc_init()
{
  ...
  ADCsync();    //  ref 31.6.16
  ADC->INPUTCTRL.bit.MUXPOS = g_APinDescription[ADCPIN1].ulADCChannelNumber;
  ADC->INPUTCTRL.bit.MUXPOS = g_APinDescription[ADCPIN2].ulADCChannelNumber; /////
  ADCsync();
...
int Joystick::getX()
{
  adc_dma(adcbuf,HWORDS,0);
  return adcbuf[0];
}

int Joystick::getY()
{
  adc_dma(adcbuf,HWORDS,1);
  return adcbuf[0];
}
